...
def lp_path_norm(model, device, p=2, input_size=[3, 32, 32]):
    ...
    data_ones = torch.ones(np.prod(input_size)).to(device)            
    if str(type(tmp_model)) == "<class 'torch.nn.parallel.data_parallel.DataParallel'>":
        if str(type(tmp_model.module))[15:18] != 'MLP':
            data_ones = torch.ones(input_size).to(device)
    elif str(type(tmp_model))[15:18] != 'MLP':
        data_ones = torch.ones(input_size).to(device)
    else:
        data_ones = data_ones.unsqueeze(0)
    try:                 
        return (tmp_model(data_ones).sum() ** (1 / p )).item()
    except:
        return (tmp_model(data_ones.unsqueeze(0)).sum() ** (1 / p )).item()
...
def calculate(trained_model, init_model, device, train_loader, margin, nchannels, nclasses, img_dim):
    ...
    #     if str(type(model.module))[15:18] != 'MLP':
    #         reparam(model)
    #         reparam(init_model)
    # elif str(type(model))[15:18] != 'MLP':
    #     reparam(model)
    #     reparam(init_model)
