...
for model in models:
    ...
    for dataset in datasets:
        ...
        for perc in percs:
            ...
            if model == "MLP":
                epochs = 150
            if model == "VGG18" or model == "ResNet18":
                epochs = 40
            ...
            if model == 'ResNet101' or model == 'WideResNet28-20':
                train_loader, test_loader, train_loader_bam, test_loader_bam, entropy, entropy_test = get_loaders(dataset, N_sub = N_sub, N2 = 0, image_aug = False, shuffle = True, BS=256)
            else:
                train_loader, test_loader, train_loader_bam, test_loader_bam, entropy, entropy_test = get_loaders(dataset, N_sub = N_sub, N2 = 0, image_aug = False, shuffle = True, shuffle_perc = perc)
            ...
            if model == "MLP":
                optimizer = torch.optim.RMSprop(my_net.parameters(), lr = LR/100)
            ...
            for epoch in range(epochs):
                ...
                for i, (x,y) in enumerate(train_loader):
                    ...
                    if epoch == epochs - 2:
                        torch.save(my_net.state_dict(), './saved_models/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'.state')
                        np.save('./results/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'_train_acc'+'.npy',train_perc)
                        np.save('./results/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'_test_acc'+'.npy',test_perc)
                        np.save('./results/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'_train_loss'+'.npy',loss_train)
                        np.save('./results/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'_test_loss'+'.npy',loss_test)
                        np.save('./results/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'_loss_gap'+'.npy',np.abs(loss_train-loss_test))
                        np.save('./results/'+model+'_'+dataset+'_shuffle'+str(int(perc*100))+'_acc_gap'+'.npy',np.abs(train_perc-test_perc))
