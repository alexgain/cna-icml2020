...
datasets = ['imagenet32','random','mnist','fashion','svhn','cifar10','cifar100']
...
for model in models:
    ...
    for dataset in datasets:
        ...
        for epoch in epochs_list:
            ...
            def margin_calc(verbose = 0, flatten=False, input_shape = 28*28):
                ...
                for i, (images, labels) in enumerate(test_loader):
                    ...
                    for k in range(labels.size(0)):
                        ...
                        margin = torch.cat((margin, output_m[:, labels].diag() - output_m[:, output_m.max(1)[1]].diag()), 0)
            ...
            # bam_val_train = bam_calc(verbose=False, flatten=flatten, input_shape=input_shape)
            # if np.isnan(bam_val):
            #     bam_val = 0
            ...
            # bam_val = train_acc(flatten=flatten, input_shape=input_shape)[0]/100 + bam_val
            # bam_val = bam_val_train - bam_val
            # bam_val = train_acc(flatten=flatten, input_shape=input_shape)[0]/100 + bam_val
            
            # bam_val = get_bam_percentile(percentile = 99.9, test = True, flatten = flatten)
            # bam_val_train = get_bam_percentile(percentile = 99.9, test = False, flatten = flatten)

            

            # print('BAM calculation test',bam_val)
            # print('BAM calculation train',bam_val_train)
            # print('BAM Difference:', bam_val - bam_val_train)

            # print('Train - BAM Difference:', train_acc_cur/100 - bam_val_train)
            
            # bam_val = bam_val + train_acc_cur
            try:
                bam_val, bam_smooth_test = get_area_measure(flatten)
            except:
                continue
            
            bam_smoothed_total.append(bam_smooth_test)
            test_smoothed_total.append(test_acc_cur)
            test_loss_total.append(test_loss_cur)
...
print(csv_array)
np.save('./results/csv_array_train.npy',csv_array)    
