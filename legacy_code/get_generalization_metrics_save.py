...
datasets = ['random','mnist','fashion','svhn','cifar10','cifar100']
source_directory = './saved_models/'

save_directory = './results/resnet101_noimagenet/'
...
for model in models:
    ...
    for dataset in datasets:
        ...
        K1 = 20 # number of models evaluated (roughly)
        if K1 == 1:
            epochs_list = [epochs-5]
        else:
            epochs_list = [0,1] + list(np.arange(5,epochs+K1,K1))
            epochs_list[-1] = -1
            
        if dataset=='imagenet32':
            # epochs_list = [0,1] + list(np.arange(10,epochs+K1,K1))
            # epochs_list[-1] = -1
            epochs_list = [0,1,20,40,-1]
            
        print(epochs_list)
        
        if dataset=='imagenet32' and model=='MLP':
            print("Skipping MLP imagenet...")
            continue

        N_sub = 0
        if model != 'ResNet101':
            train_loader, test_loader, train_loader_bam, test_loader_bam, entropy, entropy_test = get_loaders(dataset, N_sub = N_sub, image_aug = image_aug, BS = 512, BS2 = 512, shuffle = False)
        else:
        ...
        for epoch in epochs_list:
            ...
            try:
                if K1==1 or epoch == -1:
                    ...
                    if gpu_boole:
                        state_dict = torch.load(source_directory+model+'_'+dataset+'_'+'epoch'+str(epoch)+'.state')
                    else:
                        state_dict = torch.load(source_directory+model+'_'+dataset+'_'+'epoch'+str(epoch)+'.state',map_location='cpu')            
    
            except:
                print("Error loading state dict for dataset",dataset,'model',model,'epoch',epoch)
                continue
            ...
            def get_slopes_train(verbose=1, flatten=False, input_shape = 28*28):
                ...
                for i, (images, labels) in enumerate(train_loader_bam):
                    if flatten:
                    ...
                    if i%100==0 and dataset=='imagenet32':
                        print(i)
                    # images = images.no_grad()
                    # try:
                    slopes = slopes + list(my_net.beta(images))
            ...
            def margin_calc2(verbose = 0, flatten=False, input_shape = 28*28):
                ...
                t2 = time()
                if dataset=='imagenet32':
                    print('Elasped margin computation time:',(t2-t1)/60,'minutes')
                
                return val_margin
                        
            bam_val, bam_smooth_test = get_area_measure(flatten)
                        
            print('Calculating training margin...')
            print()
            ...
            measure_dict, bounds_dict = measures.calculate(my_net, init_model, torch.device("cuda" if gpu_boole else "cpu"), train_loader, tr_margin, channels, num_classes, img_dim)
            
            # try:
            #     bam_val, bam_smooth_test = get_area_measure(flatten)
            # except:
            #     print('Error in bam-area calculation, skipping...')
            #     continue
            
            bam_val_margin = copy.copy(bam_val)
            
            print('BAM-area:',bam_val_margin)                        
            ...
            bam_.append((model+' '+dataset+' '+str(epoch)+' ',bam_smooth_test))
            bam_area_.append((model+' '+dataset+' '+str(epoch)+' ',bam_val))
            bam_margin_.append((model+' '+dataset+' '+str(epoch)+' ',bam_val_margin))
            margin_.append((model+' '+dataset+' '+str(epoch)+' ',tr_margin))
            l1_.append((model+' '+dataset+' '+str(epoch)+' ',get_l1_norm(my_net)))
            l2_.append((model+' '+dataset+' '+str(epoch)+' ',copy.copy(measure_dict['Frobenious norm'])))
            l2path_.append((model+' '+dataset+' '+str(epoch)+' ',copy.copy(measure_dict['L2_path norm'])))
            spectral_.append((model+' '+dataset+' '+str(epoch)+' ',copy.copy(measure_dict['Spectral norm'])))
            bound2018_.append((model+' '+dataset+' '+str(epoch)+' ',copy.copy(bounds_dict['Spec_Fro Bound (Neyshabur et al. 2018)'])))
            train_acc_.append((model+' '+dataset+' '+str(epoch)+' ',train_acc_cur))
            train_loss_.append((model+' '+dataset+' '+str(epoch)+' ',train_loss_cur))
            test_acc_.append((model+' '+dataset+' '+str(epoch)+' ',test_acc_cur))
            test_loss_.append((model+' '+dataset+' '+str(epoch)+' ',test_loss_cur))
            gap_acc_.append((model+' '+dataset+' '+str(epoch)+' ',gen_gap_acc))
            gap_loss_.append((model+' '+dataset+' '+str(epoch)+' ',gen_gap_loss))
            
            print('Cur bam_ list:',bam_margin_)
            print('Cur gap_acc_ list:',gap_loss_)
            
            np.save(save_directory+'bam_.npy',np.array(bam_))
            np.save(save_directory+'bam_area_.npy',np.array(bam_))
            ...
            t2 = time()
            print('Elapsed time for epoch evaluation:',(t2-t1)/60,'minutes')
            print()
